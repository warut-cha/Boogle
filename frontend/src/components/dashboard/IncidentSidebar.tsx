...
export function IncidentSidebar({
  ...
  const statusLabel =
    status === "connected" ? "Sentinel · live" : status === "connecting" ? "Sentinel · linking" : "Sentinel · offline";
  return (
    <aside data-annotate="sidebar" className="w-[300px] shrink-0 border-r border-border-dim flex flex-col bg-background">
      ...
      <div className="p-3 border-t border-border-dim grid grid-cols-2 gap-2">
        <button
          onClick={onScan}
          className="text-[10px] font-mono tracking-widest uppercase text-muted-foreground hover:text-foreground transition-colors py-2 border border-border-dim hover:border-border"
        >
          + Scan
        </button>
        <button
          onClick={onSimulate}
          className="text-[10px] font-mono tracking-widest uppercase text-muted-foreground hover:text-critical transition-colors py-2 border border-border-dim hover:border-critical/50"
        >
          Simulate
        </button>
      </div>
